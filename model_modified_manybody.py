...
#        sin_theta_s2 = sin_theta_s * sin_theta_s
    ...
    #            tf.debugging.check_numerics(evdw, message='Total_energy_vdw contains NaN')
            ...
            #gauss_term_fcut = tf.einsum('ijk,ij->ijk',gauss_term,fcut)
            #atomic_descriptors = tf.einsum('ijk,ijl->ikl',gauss_term_fcut, species_encoder_ij) # nat,Nrs,Nneigh,nembeding
            ...
            radial_ij = tf.einsum('ijk,ijl->ijkl',bf_radial_ang,species_encoder_ij)
            radial_ij = tf.reshape(radial_ij, [nat,Nneigh,Ngauss_ang*self.nspec_embedding])
    ...
    def call(self, inputs, training=False):
        ...
        #energies_forces = tf.vectorized_map(self.tf_predict_energy_forces, elements)

        #energies = energies_forces[0]
        #forces = tf.reshape(forces, [-1,3])

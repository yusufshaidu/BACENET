...
@tf.function(jit_compile=False,
            ...
            input_signature=[(
            tf.TensorSpec(shape=(None,), dtype=tf.float32), #0 - species_encoder
            tf.TensorSpec(shape=(None,), dtype=tf.float32), #1 - positions
            tf.TensorSpec(shape=(), dtype=tf.int32),        #2 - nmax_diff, difference between nat_max and nat
            tf.TensorSpec(shape=(), dtype=tf.int32),        #3 - nat
            tf.TensorSpec(shape=(None,), dtype=tf.float32), #4 - cell 
            tf.TensorSpec(shape=(None,), dtype=tf.float32), #5 - C6
            tf.TensorSpec(shape=(None,), dtype=tf.int32),   #6 - first_atom_idx
            tf.TensorSpec(shape=(None,), dtype=tf.int32),   #7 - second_atom_idx
            tf.TensorSpec(shape=(None,), dtype=tf.int32),   #8 - shift_vectors
            tf.TensorSpec(shape=(), dtype=tf.int32),        #9 - num_pairs
            tf.TensorSpec(shape=(None,), dtype=tf.float32), #10 - gaussian_width
            tf.TensorSpec(shape=(None,), dtype=tf.float32), #11 - chi0
            tf.TensorSpec(shape=(None,), dtype=tf.float32), #12 - J0
            tf.TensorSpec(shape=(None,), dtype=tf.float32), #13 - atomic_q0
            tf.TensorSpec(shape=(), dtype=tf.float32),      #14 - total charge
            tf.TensorSpec(shape=(None,), dtype=tf.float32), #15 - nuclei_charge
            tf.TensorSpec(shape=(None,), dtype=tf.int32),   #16 - atomic_number
            tf.TensorSpec(shape=(), dtype=tf.float32),      #17 - rc
            tf.TensorSpec(shape=(), dtype=tf.int32),        #18 - nrad
            tf.TensorSpec(shape=(3,), dtype=tf.float32),    #19 - efield
            tf.TensorSpec(shape=(), dtype=tf.int32),        #20 - body_order
            )])
def tf_predict_energy_forces(self,x):
    ...
    with tf.GradientTape(persistent=True) as tape0:
        ...
        for nbody in tf.range(3, body_order+1):
        ...
        Pi_a = Piq_a + Pie_a
        efield_energy = -tf.reduce_sum(Pi_1 * _efield)
        ecoul += efield_energy 
        total_energy += ecoul

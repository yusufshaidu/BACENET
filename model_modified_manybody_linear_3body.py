...
class mBP_model(tf.keras.Model):
    ...
    def __init__(self, layer_sizes, 
       ...
       # create a species embedding network with 1 hidden layer Nembedding x Nspecies
        ...
        inputs = tf.ones(self.thetaN)
        self.lambda_1 = Networks(self.thetaN, 
                            [64,self.thetaN], 
                            ['sigmoid','sigmoid'],
                            bias_initializer='ones',
                            prefix='lambda_1')[inputs]
        self.lambda_2 = Networks(self.thetaN, 
                            [64,self.thetaN], 
                            ['tanh','tanh'],
                            bias_initializer='ones',
                            prefix='lambda_2')[inputs]

        self.radial_funct_net = Networks(self.Nrad, [64,self.Nrad], ['sigmoid','linear'], 
    ...
    #            tf.debugging.check_numerics(evdw, message='Total_energy_vdw contains NaN')
            ...
            zeta_float = tf.cast(zeta, tf.float32)
            zeta_fact = tf.exp(tf.math.lgamma(zeta_float+1.0))

            reg = 1e-20
            all_rij_norm_inv = 1.0 / (all_rij_norm + reg) #n, nneigh

            #rij_unit = tf.einsum('ijk,ij->ijk',all_rij, all_rij_norm_inv)
            
            Gi3 = [] 
            for n in range(zeta):
                n_float = tf.cast(n, tf.float)
                _lambda_1 = self.lambda_1 ** (zeta_float-n_float)
                _lambda_2 = self.lambda_2 ** n_float
                lambda_12 = _lambda_1 * _lambda_2
                
                n_fact = tf.exp(tf.math.lgamma(n_float+1.0))
                z_n_fact = tf.exp(tf.math.lgamma(zeta_float-n_float,tf.float32)+1.0))
                z_comb_n = zeta_fact / (z_n_fact * n_fact)

                rij_norm_n = all_rij_norm_inv ** n_float
                pqr_indices = help_fn.find_three_non_negative_integers(n)

                #lx!ly!lz!
                fact_lxlylz = tf.reshape(tf.map_fn(help_fn.factorial, tf.reshape(pqr_indices, [-1]), 
                ...
                rij_lxlylz = (all_rij[:,:,tf.newaxis,:] + 1e-16)**(tf.cast(pqr_indices, tf.float32)[tf.newaxis,tf.newaxis,:,:])
                rij_lxlylz = nfact_lxlylz[None,None,:] * tf.reduce_prod(rij_lxlylz, axis=-1) # nat, nneigh, Mn
                gij_lxlylz = radial_ij * rij_lxlylz[:,:,None,None,:] * rij_norm_n[:,:,None,None,None] # nat, neigh, Nrad, nembedding**2, Mn
                gi_lxlylz = z_comb_n * tf.reduce_sum(gij_lxlylz, axis=1) # nat, Nrad, nembedding**2, Mn
                gi_3 = tf.einsum('ijkl, m->ijkm', gi_lxlylz * gi_lxlylz, lambda_12) #nat,Nrad, nembedding**2,thetaN
                Gi3.append(gi_3)
            #removing j = k components
            term_j_k = 2.0 * tf.reduce_sum(radial_ij * radial_ij, axis=1) # nat,Nrad, nembedding**2
            Gi3 = tf.reduce_sum(Gi3, axis=0) * 2.0**(1-zeta_float) - term_j_k[:,:,:,None]
    ...
    def train_step(self, data):
        ...
        with self.train_writer.as_default(step=self._train_counter):
            ...
            tf.summary.histogram(f'4. angular terms: lambda 1',self.lambad_1,self._train_counter)
            tf.summary.histogram(f'4. angular terms: lambda 2',self.lambad_2,self._train_counter)
            for idx, spec in enumerate(self.species_identity):
